...
if (!class_exists('WPSAccount')) :
	class WPSAccount {
		...
		public function authenticatedUrl($method) {
			$bvinfo = new WPSInfo($this->settings);
		...
		public static function removeByAccountType($settings, $account_type) {
			$accounts = WPSAccount::accountsByType($settings, $account_type);
			if (sizeof($accounts) >= 1) {
				foreach ($accounts as $pubkey => $value) {
					WPSAccount::remove($settings, $pubkey);
				}
				return true;
			}
			return false;
		...
		public static function removeByAccountGid($settings, $account_gid) {
			$accounts = WPSAccount::accountsByGid($settings, $account_gid);
			if (sizeof($accounts) >= 1) {
				foreach ($accounts as $pubkey => $value) {
					WPSAccount::remove($settings, $pubkey);
				}
