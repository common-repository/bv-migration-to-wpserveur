...
class WPSWPAdmin {
	...
	public function initHandler() {
		...
		if (array_key_exists('bvnonce', $_REQUEST) &&
			...
			WPSAccount::addAccount($this->settings, $keys[0], $keys[1]);
	...
	public function settingsLink($links, $file) {
		if ( $file == plugin_basename( dirname(__FILE__).'/wpserveur.php' ) ) {
			$links[] = '<a href="'.$this->mainUrl().'">'.__( 'Settings' ).'</a>';
		}
		return $links;
	}

	public function wpssecAdminMenu($hook) {
		if ($hook === 'toplevel_page_wpserveur') {
			wp_register_style('wpssurface', plugins_url('css/form-styles.css', __FILE__ ));
			wp_enqueue_style('wpssurface');
		}
	}
	...
	public function siteInfoTags() {
		...
		$secret = WPSRecover::defaultSecret($this->settings);
		$public = WPSAccount::getApiPublicKey($this->settings);
	...
	public function activateWarning() {
		global $hook_suffix;
		if (!WPSAccount::isConfigured($this->settings) && $hook_suffix == 'index.php' ) {
?>
			<div id="message" class="updated" style="padding: 8px; font-size: 16px; background-color: #dff0d8">
						<a class="button-primary" href="<?php echo $this->mainUrl(); ?>">Activer la migration de WPServer</a>
						&nbsp;&nbsp;&nbsp;<b>Presque fini:</b> Activer votre compte WPServeur pour migrer votre site.
			</div>
<?php
		}
	}
